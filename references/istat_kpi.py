...
def new_index(old_df, lista_col_num, lista_col_den, nome_indice):
    ...
    old_df[nome_indice] = old_df[lista_col_num].sum(
        axis=1) / old_df[lista_col_den].sum(axis=1)
...
def wrangle_istat_cpa2011(quartiere_data, selectedCity):
    ...
    list_columns_continenti = [
        'perc_europei', 'perc_africa', 'perc_america', 'perc_asia', 'perc_oceania']
    ...
    for idx, continente in enumerate(list_columns_continenti):
        new_index(quartiere_data, [list_attr_continenti[idx]], [
                  'ST15'], list_columns_continenti[idx])

    # 3. Creo la nuova colonna
    new_index(quartiere_data, ['P43', 'P44', 'P45'], [
              'P30', 'P31', 'P32'], 'indice_vecchiaia_uomo')
    # Creo la nuova colonna
    new_index(quartiere_data, ['P27', 'P28', 'P29'], [
              'P14', 'P15', 'P16'], 'indice_vecchiaia')
    # Creo la nuova colonna
    new_index(quartiere_data, ['P43', 'P44', 'P45'], [
              'P30', 'P31', 'P32'], 'indice_vecchiaia_uomo')
    # Calcola numeratore e denominatore
    donne_anziane = quartiere_data[['P27', 'P28', 'P29']].sum(
        axis=1) - (quartiere_data[['P43', 'P44', 'P45']].sum(axis=1))
    donne_giovani = quartiere_data[['P14', 'P15', 'P16']].sum(
        axis=1) - quartiere_data[['P30', 'P31', 'P32']].sum(axis=1)
    # Quindi, il nuovo indice
    quartiere_data['indice_vecchiaia_donna'] = donne_anziane / donne_giovani

    # 4. Creo la nuova colonna
    new_index(quartiere_data, ['P22', 'P23', 'P24', 'P25', 'P26'], [
              'P17', 'P18', 'P19', 'P20', 'P21'], 'indice_pop_attiva')
    # Creo la nuova colonna
    new_index(quartiere_data, ['P38', 'P39', 'P40', 'P41', 'P42'], [
              'P33', 'P34', 'P35', 'P36', 'P37'], 'indice_pop_attiva_uomo')
    # Calcolo numeratore e denominatore
    numeratore_donne = (quartiere_data[['P22', 'P23', 'P24', 'P25', 'P26']].sum(
        axis=1)) - (quartiere_data[['P38', 'P39', 'P40', 'P41', 'P42']].sum(axis=1))
    denominatore_donne = (quartiere_data[['P17', 'P18', 'P19', 'P20', 'P21']].sum(
        axis=1)) - (quartiere_data[['P33', 'P34', 'P35', 'P36', 'P37']].sum(axis=1))
    # Creazione colonna
    quartiere_data['indice_pop_attiva_donna'] = numeratore_donne / \
        denominatore_donne
    ...
    new_index(quartiere_data, ['P137'], ['P1'],
              'indice_pop_non_pend_esterna_quartiere')

    # 7. Creo nuova colonna
    giornata_dentro_quartiere = quartiere_data['P1'] - \
        (quartiere_data[['P137', 'P138']].sum(axis=1))
    quartiere_data['indice_pop_non_pend_interna_quartiere'] = giornata_dentro_quartiere / \
        quartiere_data['P1']
