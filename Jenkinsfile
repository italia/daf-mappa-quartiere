pipeline {
  ...
  stage('Deploy prod') {
    ...
    steps {
      ...
      sh ''' kubectl --kubeconfig='/home/centos/.kube/config.teamdigitale-production' delete -f mappa-quartiere1.yaml'''
      sh ''' kubectl --kubeconfig='/home/centos/.kube/config.teamdigitale-production' create -f mappa-quartiere1.yaml'''
      slackSend (color: '#00FF00', message: "SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' https://cd.daf.teamdigitale.it/blue/organizations/jenkins/daf-mappa-quartiere/activity")
    }
  }
}
