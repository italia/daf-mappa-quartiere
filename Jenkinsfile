pipeline {
    ...
    stage('Deploy test') {
      ...
      steps {
        sh ''' COMMIT_ID=$(echo ${GIT_COMMIT}|cut -c 1-6);
              sed "s#image: nexus.teamdigitale.test/daf-mappa.*#image: nexus.teamdigitale.test/daf-mappa-quartiere:$BUILD_NUMBER-$COMMIT_ID#" mappa-quartiere.yaml > mappa-quartiere1.yaml ;kubectl replace -f mappa-quartiere1.yaml --validate=false'''
  ...
  stage('Deploy prod') {
    ...
    steps {
      sh ''' COMMIT_ID=$(echo ${GIT_COMMIT}|cut -c 1-6);
              sed "s#image: nexus.daf.teamdigitale.it/daf-mappa.*#image: nexus.daf.teamdigitale.it/daf-mappa-quartiere:$BUILD_NUMBER-$COMMIT_ID#" mappa-quartiere.yaml > mappa-quartiere1.yaml'''
      sh ''' kubectl --kubeconfig='/home/centos/.kube/config.teamdigitale-production' delete -f mappa-quartiere1.yaml --validate=false'''
      sh ''' kubectl --kubeconfig='/home/centos/.kube/config.teamdigitale-production' create -f mappa-quartiere1.yaml --validate=false'''
      slackSend (color: '#00FF00', message: "SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' https://cd.daf.teamdigitale.it/blue/organizations/jenkins/daf-mappa-quartiere/activity")
    }
  }
}
