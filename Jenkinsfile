pipeline {
  ...
  stages {
    stage('Build') {
      steps { //sh1 != sh2  can't pass env var
    ...
    stage('Upload'){
      steps {
        script {
          if(env.BRANCH_NAME == 'production'){ //push on nexus private repo
      ...
      steps {
        script {
          if(env.BRANCH_NAME == 'production'){
            ...
            sh "sed 's/daf-mappa-quartiere*/daf-mappa-quartiere:$BUILD_NUMBER-$COMMITID/g' files.yaml"
